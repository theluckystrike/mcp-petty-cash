...
import { dataDir, findStatement, getStatements, lockPath, nextStatementId, setStatements, statementKey, } from "./store.js";
import { businessMissing, degradedNotes, invoicesUnreadable, issuer, NO_BUSINESS_NOTE, readSources, sourceReport, } from "./sources.js";
import { ageClient, buildStatement, BUCKETS, oldestOverdue, pickCurrency, resolveClient, } from "./statement.js";
/**
...
function checkDate(value, fallback, field) {
    ...
    if (!isIsoDate(d))
        throw new Error(`cannot read a date: ${field} "${value}" is not a real date in YYYY-MM-DD form.`);
    return d;
}
/* ----------------------------------------------------------------- assembly */
/** Read the stores once per call and refuse by name when the invoice ledger is unreadable. */
function sources(toolName) {
    const s = readSources();
    if (!s.invoices.ok)
        throw new Error(invoicesUnreadable(s, toolName));
    return s;
}
function scopeFor(s, client, currency) {
...
function statementJson(st, s, record) {
    return {
        ...
        basis: "closing = opening + invoices issued - payments received - credit notes. Payments received ALREADY includes any deposit applied to an " +
...
function capRefusal(count, month, toolName) {
 ...
 * pass it.
 */
async function register(st, toolName) {
    return await locked(() => {
        ...
        if (!gate.isPro()) {
            ...
            if (built >= FREE_STATEMENTS_PER_MONTH)
                throw new Error(capRefusal(built, month, toolName));
        }
        const rec = {
...
/* ------------------------------------------------------------------- server */
const server = new McpServer({ name: "mcp-statement-of-account", version: VERSION }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
    ...
    try {
        ...
        for (const scope of scopes) {
            ...
            for (const cur of currencies) {
                if (!scope.currencies.includes(cur))
                    continue;
                const aged = ageClient(scope, cur, asOf);
                if (!aged.outstanding_minor && !aged.unapplied_credit_minor)
                    continue;
        ...
        return json({
            ...
            basis: "Every figure is as at as_of: an invoice issued after that date is not on the books, a payment made after it has not happened, and a credit " +
...
function statementLines(st, day, greeting, signOff) {
    const biz = issuer();
    ...
    out.push("");
    out.push(st.closing_minor > 0 ? `${money(st.closing_minor, st.currency)} is outstanding on this account.`
...
server.registerTool("statement_pdf", {
    ...
    try {
        if (!gate.isPro())
            return fail(gate.upgradeText("the statement of account PDF", "statement_pdf"));
...
function bankLines() {
    ...
    const out = [];
    if (p.bank?.trim())
        out.push(`Bank:  ${p.bank.trim()}`);
    if (p.iban?.trim())
        out.push(`IBAN:  ${p.iban.trim()}`);
    if (p.vat_id?.trim())
        out.push(`VAT:   ${p.vat_id.trim()}`);
    return out;
}
server.registerTool("dunning_text", {
    ...
    try {
        const level = a.level;
        if (level === 3 && !gate.isPro())
            return fail(gate.upgradeText("the level 3 final demand", "dunning_text"));
        ...
        if (!overdue.length) {
            return fail(`nothing to chase: ${scope.name} has nothing overdue in ${currency} as at ${day}` +
        ...
        if (!bank.length) {
            extra.push("No bank or IBAN is in the shared business profile, so the letter asks for payment without saying where to send it. " +
                "Run business_set {iban, bank} in the invoice server (mcp-invoice) and write it again.");
        }
        extra.push(`Level ${level} (${L.label}). No late fee, interest or cost is stated: this server holds no contract terms, no statutory ` +
...
server.registerTool("statements_report", {
    ...
    try {
        if (!gate.isPro())
            return fail(gate.upgradeText("the receivables report", "statements_report"));
        ...
        for (const name of clientNames(s)) {
            ...
            for (const cur of scope.currencies) {
                ...
                if (!aged.outstanding_minor)
                    continue;
                everyInvoice.push(...aged.invoices);
                const b = perCurrency.get(cur) ?? { outstanding: 0, overdue: 0, buckets: { "0-30": 0, "31-60": 0, "61-90": 0, "over 90": 0 }, clients: new Set() };
                b.outstanding += aged.outstanding_minor;
                b.overdue += aged.overdue_minor;
                for (const k of BUCKETS)
                    b.buckets[k] += aged.buckets[k];
        ...
        return json({
            ...
            basis: "One total per currency and never one across them: this server holds no exchange rate, so a single number over a EUR ledger and a USD one " +
